...
@file: mydataset2.py
@time: 2021/2/10 15:48
@desc: For dataset without testing set.
'''
import os
...
class build_mydl(LightningDataModule):
    ...
    def train_dl(self):
        ...
        if self.depth == 0:
            ...
            val_sub = subset_3
            ds_list.remove(subset_3)
            subset = ConcatDataset(ds_list[0:4])
        elif self.depth == 2:
            val_sub = subset_2
            ds_list.remove(subset_2)
            subset = ConcatDataset(ds_list[0:4])
        elif self.depth == 3:
            val_sub = subset_1
            ds_list.remove(subset_1)
            subset = ConcatDataset(ds_list[0:4])
        elif self.depth == 4:
            val_sub = subset_0
            ds_list.remove(subset_0)
            subset = ConcatDataset(ds_list[0:4])
    ...
    #             2.Dataloader
    #             """
    #     subset_1, subset_2, subset_3, subset_4, subset_5, _ = self.__getDS()
    #
    #     if self.depth == 0:
    #         subset = subset_5
    #     elif self.depth == 1:
    ...
    #     elif self.depth == 4:
    #         subset = subset_1
    #     dl_val = data.DataLoader(dataset=subset,
    #                                batch_size=self.batch_size,
    #                                shuffle=False)
    #     return dl_val
    ...
    #     2.Dataloader
    #     """
    #     transform = self.default_transforms() #if self.test_transforms is None else self.test_transforms
    #     ds_test = ImageFolder(self.test_PATH,
    #                           transform=transform)
    #     dl_test = DataLoader(
    #         ds_test,
    #         batch_size=self.batch_size,
